...
A React + Vite + Tailwind front-end (already designed on claude design; source lives in `ui/web/` and compiles to the static `public/` directory) hosted on **Cloudflare Pages**, backed by three **Pages Functions** that implement an **Asana OAuth 2.0 authorization-code flow**. Each user logs in as themselves; the app acts with their own Asana permissions and shows their own view. Access is restricted to members of one specific Asana workspace (Inner Excellence) so the public internet cannot use the deployment even though there is no per-user allowlist.

Explicitly **not** using: standalone Workers, `wrangler`-based Worker deploys, KV, D1, or any database, server-side rendering, or a Node server at runtime — the front-end build output is static files. Tokens live in HttpOnly cookies. Pages Functions are the only server-side code, and they deploy automatically with the site (no separate deploy step).
...
/
├─ ui/
│  └─ web/                     # React + Vite + Tailwind source for the web UI
│     ├─ index.html
│     └─ src/
│        ├─ main.jsx
│        ├─ App.jsx
...
directory (set "Build output directory" = `public` in the Pages project settings; the
Pages project's own build command should run `npm run build`, which runs `vite build`
into `public/` before Cloudflare picks up `functions/`).
...
The coding model should wire the React UI (component structure and visuals from the
imported claude-design project) to these conventions; do not redesign the visuals.
...
1. Create a Pages project connected to the repo. Build command: `npm run build`. Build output
   directory: `public`.
...
3. The React UI implementing the imported claude-design components, wired to the
   `fetch`/401/logout conventions above, with no visual redesign beyond what's needed
   for real data.
